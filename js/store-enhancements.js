...
function populateCartDrawer() {
  ...
  cart.forEach((item, index) => {
    ...
    itemsHtml += `
      <div class="drawer-cart-item fade-in" style="animation-delay: ${
        index * 50
      }ms">
        ...
        <div class="drawer-item-controls">
          <div class="drawer-qty-controls">
            <button onclick="updateCartQtyFromDrawer(${
          ...
          <button class="drawer-remove-btn" onclick="removeFromCartDrawer(${
            item.product_id || item.id
          })" title="Hapus">
...
async function showProductDetail(productId) {
  try {
    ...
    if (pdImage) {
      pdImage.src =
        currentProduct.image_url || "assets/images/product-placeholder.png";
    ...
    const hasMemberPrice =
      ...
      currentProduct.member_price < currentProduct.price;
    if (pdPricing) {
      if (hasMemberPrice) {
        pdPricing.innerHTML = `
          <div class="pd-member-price-section">
            <span class="pd-member-label">Harga Member</span>
            <span class="pd-member-price">Rp ${formatPrice(
              currentProduct.member_price
            )}</span>
          </div>
          <div class="pd-normal-price strikethrough">Rp ${formatPrice(
            currentProduct.price
          )}</div>
        `;
      } else {
        pdPricing.innerHTML = `<div class="pd-normal-price">Rp ${formatPrice(
          currentProduct.price
        )}</div>`;
      }
    }
...
function addToCartFromDetail() {
  ...
  const isLoggedIn = !!sessionStorage.getItem("userId");
  const price =
    isLoggedIn && currentProduct.member_price
      ? currentProduct.member_price
      : currentProduct.price;
...
async function openCheckoutModal() {
  ...
  try {
    ...
    if (result.loggedIn && result.user) {
      ...
      // Sync order type
      const drawerOrderType = document.querySelector(
        'input[name="drawerOrderType"]:checked'
      );
      if (drawerOrderType) {
        const mainOrderType = document.querySelector(
          `input[name="orderType"][value="${drawerOrderType.value}"]`
        );
...
async function confirmCheckout() {
  ...
  if (!isLoggedIn) {
    ...
    if (mainGuestAddress)
      mainGuestAddress.value =
        document.getElementById("checkout-guest-address")?.value || "";
  }
...
function updateDrawerTotalWithDiscount() {
  ...
  if (typeof storeCouponData !== "undefined" && storeCouponData) {
    ...
    if (
      storeCouponData.discountType === "free_product" &&
      storeCouponData.targetProductId
    ) {
      ...
      discountAmount = Math.round(
        (subtotal * (storeCouponData.discountValue || 0)) / 100
      );
    } else {
      discountAmount = parseFloat(
        storeCouponData.discountAmount || storeCouponData.discountValue || 0
      );
    }
...
function renderEnhancedProductCard(product) {
  ...
  return `
    <div class="product-card-enhanced hover-lift" data-product-id="${
      product.id
    }">
      <div class="product-image-wrapper">
        ...
        <div class="product-badges">
          ${
            product.promo_text
              ? `<span class="promo-badge">${escapeHtml(
                  product.promo_text
                )}</span>`
              : "<span></span>"
          }
      ...
      <div class="product-info">
        ...
        <div class="product-pricing">
          ${
            ...
            <div class="member-price-wrapper">
              ...
              <span class="member-price">Rp ${formatPrice(
                product.member_price
              )}</span>
            </div>
            <span class="normal-price strikethrough">Rp ${formatPrice(
              product.price
            )}</span>
          `
              : `
        ...
        <div class="product-actions">
          <button 
            ...
            onclick="addToCartEnhanced(${product.id}, '${escapeJsString(
    product.name
  )}', ${product.price}, ${product.member_price || "null"})"
...
function renderHistoryCards(activities) {
  ...
  container.innerHTML = activities
    .map((activity) => {
      ...
      const statusBadge = getHistoryStatusBadge(
        activity.status,
        isOrder ? "order" : "reward"
      );
...
async function checkPendingOrdersOnLoad() {
  try {
    ...
    const response = await fetch(
      "/api/user-data/activities?page=1&limit=10&type=order",
      {
        credentials: "include",
      }
    );
        ...
        <div class="history-detail-body">
          ...
          <div class="history-order-info">
            <div class="history-order-row">
              ...
              <span class="value" id="historyOrderNumber">#${escapeHtml(
                order.order_number
              )}</span>
            </div>
            <div class="history-order-row">
          ...
          <div class="history-detail-actions">
            <button class="btn btn-primary" id="historyCheckStatusBtn" onclick="checkHistoryOrderStatus(${
              order.id
            })">
...
async function openHistoryRewardDetail(redemptionId) {
  try {
    ...
    if (reward.coupon_code) {
      ...
      couponCodeHtml = `
        <div class="history-order-row" style="flex-direction: column; gap: 8px; margin-top: 16px; padding: 16px; background: ${
          canCopy ? "#F0FDF4" : "#FEF2F2"
        }; border-radius: 12px;">
          <span class="label" style="font-size: 12px; color: ${
            canCopy ? "#166534" : "#991B1B"
          };">🎫 Kode Voucher Anda:</span>
          <div style="display: flex; gap: 8px; align-items: center;">
            <code style="flex: 1; padding: 12px 16px; background: white; border: 2px dashed ${
              canCopy ? "#22C55E" : "#DC2626"
            }; border-radius: 8px; font-size: 18px; font-weight: 700; color: ${
        canCopy ? "#166534" : "#991B1B"
        ...
        reward.coupon_code
          ...
          ${
            ...
            <p style="font-size: 11px; color: ${
              displayStatus === "used" ? "#166534" : "#DC2626"
            }; margin-top: 4px;">
              ${warningMsg}
            </p>
          `
      ...
      <div class="history-detail-body">
        <div class="history-order-info">
          ...
          <div class="history-order-row">
            ...
            <span class="value" style="color: #B45309;">-${
              reward.points_cost
            } poin</span>
          </div>
          <div class="history-order-row">
            ...
            }">${expiryStr} ${isExpired ? "(Kadaluarsa)" : ""}</span>
          </div>
          <div class="history-order-row">
            <span class="label">Status</span>
            <span class="value">${getHistoryStatusBadge(
              displayStatus,
              "reward"
            )}</span>
          </div>
...
async function checkHistoryOrderStatus(orderId) {
  ...
  if (checkBtn) {
    ...
    checkBtn.innerHTML =
      '<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Mengecek...';
    if (typeof lucide !== "undefined") {
      lucide.createIcons();
    }
  }
  ...
  try {
    ...
    if (order.status === "completed" || order.status === "paid") {
      ...
      if (typeof showToast === "function") {
        showToast(
          "✅ Pesanan sudah di-accept! Poin Anda telah ditambahkan.",
          "success"
        );
      }
    ...
    if (checkBtn) {
      ...
      checkBtn.innerHTML =
        '<i data-lucide="refresh-cw" class="w-4 h-4"></i> Cek Status Pesanan';
      if (typeof lucide !== "undefined") {
        lucide.createIcons();
      }
    }
